...
const generarID = (prefijo) => {
  const uuid = 'xxxxxxxx'.replace(/[x]/g, () => Math.floor(Math.random() * 16).toString(16));
  return prefijo + '_' + uuid.toUpperCase();
};
...
// CARGA INICIAL
// ============================================
router.get('/datos/:empresaID', async (req, res) => {
  try {
  ...
  try {
    ...
    await db.query(
      `INSERT INTO Productos (ProductoID, EmpresaID, ProveedorID, NombreProducto, PuntoVentaNombre, 
        ...
        PermiteDescuento, DescuentoMax, Activo, FechaRegistro)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, NOW())`,
      [productoID, data.empresaID, data.proveedorID || null, data.nombreProducto, 
       data.puntoVentaNombre || data.nombreProducto, data.codigoBarras || null, data.imagen || null,
       data.unidadCompra || 'PZ', data.contenidoUnidadCompra || 1, data.unidadVenta || 'PZ',
       data.precio1 || 0, data.precio2 || 0, data.precio3 || 0, data.precio4 || 0, data.precio5 || 0, data.precio6 || 0,
...
router.delete('/productos/:id', async (req, res) => {
  try {
    const { id } = req.params;
    await db.query('UPDATE Productos SET Activo = 0 WHERE ProductoID = ?', [id]);
    res.json({ success: true });
  } catch (error) {
    res.json({ success: false, error: error.message });
  }
...
router.post('/clientes', async (req, res) => {
  try {
    const data = req.body;
    const clienteID = generarID('CLI');
    
    await db.query(
      `INSERT INTO Clientes (ClienteID, EmpresaID, Nombre, Telefono, TipoPrecio, Credito, LimiteCredito, Activo, FechaRegistro)
       VALUES (?, ?, ?, ?, ?, ?, ?, 1, NOW())`,
      [clienteID, data.empresaID, data.nombre, data.telefono || null, data.tipoPrecio || 1, data.credito || 0, data.limiteCredito || 0]
    );
...
router.delete('/clientes/:id', async (req, res) => {
  try {
    const { id } = req.params;
    await db.query('UPDATE Clientes SET Activo = 0 WHERE ClienteID = ?', [id]);
    res.json({ success: true });
  } catch (error) {
    res.json({ success: false, error: error.message });
  }
  ...
  try {
    ...
    await db.query(
      `INSERT INTO Ventas (VentaID, EmpresaID, SucursalID, ClienteID, UsuarioEmail, FechaHora, TipoPrecio, Total, TipoVenta, Estatus)
       VALUES (?, ?, ?, ?, ?, NOW(), ?, ?, ?, ?)`,
      [ventaID, data.empresaID, data.sucursalID, data.clienteID || null, data.usuarioEmail, 
    ...
    for (const item of data.items) {
      ...
      await db.query(
        `INSERT INTO DetalleVenta (DetalleID, VentaID, ProductoID, Cantidad, PrecioUnitario, DescuentoPct, DescuentoMonto, Subtotal, FechaHora)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
        [detalleID, ventaID, item.productoID, item.cantidad, item.precioUnitario, item.descuentoPct || 0, item.descuentoMonto || 0, item.subtotal]
      );
    }
    ...
    if (data.pagos && data.pagos.length > 0) {
      for (const pago of data.pagos) {
        if (pago.monto > 0) {
          const abonoID = generarID('ABO');
          await db.query(
            `INSERT INTO Abonos (AbonoID, VentaID, EmpresaID, MetodoPagoID, Monto, UsuarioEmail, FechaHora)
             VALUES (?, ?, ?, ?, ?, ?, NOW())`,
            [abonoID, ventaID, data.empresaID, pago.metodoPagoID, pago.monto, data.usuarioEmail]
          );
        }
      }
    }
...
router.get('/ventas/dia/:empresaID/:sucursalID', async (req, res) => {
  try {
    const { empresaID, sucursalID } = req.params;
    
    const [ventas] = await db.query(
      `SELECT * FROM Ventas WHERE EmpresaID = ? AND SucursalID = ? AND DATE(FechaHora) = CURDATE()`,
...
router.get('/ventas/espera/:empresaID/:sucursalID', async (req, res) => {
  try {
    const { empresaID, sucursalID } = req.params;
    
    const [ventas] = await db.query(
      `SELECT * FROM Ventas WHERE EmpresaID = ? AND SucursalID = ? AND Estatus = 'EN_ESPERA'`,
      [empresaID, sucursalID]
    );
...
router.get('/ventas/ticket/:empresaID/:sucursalID', async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      `SELECT COUNT(*) as total FROM Ventas WHERE EmpresaID = ? AND SucursalID = ? AND DATE(FechaHora) = CURDATE()`,
      [empresaID, sucursalID]
    );
